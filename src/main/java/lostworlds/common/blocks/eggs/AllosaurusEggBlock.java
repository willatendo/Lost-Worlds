...
public class AllosaurusEggBlock extends AbstractDinosaurEgg
{
	@Override
	...
	public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random rand) 
	{
		if(this.shouldUpdateHatchLevel(world) && onNest(world, pos)) 
		{
			...
			else 
			{
				...
				AllosaurusEntity allosaurus = EntityInit.ALLOSAURUS_ENTITY.get().create(world);
				allosaurus.moveTo((double)pos.getX() + 0.3D * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
				world.addFreshEntity(allosaurus);
				allosaurus.setAge(-24000);
			}
		}
	}
}

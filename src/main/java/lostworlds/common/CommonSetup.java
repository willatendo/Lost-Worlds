...
public class CommonSetup 
{
	...
	public static void registerEntityAttributes(FMLCommonSetupEvent event) 
	{
		event.enqueueWork(() -> 
		{
			...
			GlobalEntityTypeAttributes.put(EntityInit.FUKUIVENATOR_ENTITY.get(), MobEntity.createMobAttributes().add(Attributes.MAX_HEALTH, 15.0D).add(Attributes.MOVEMENT_SPEED, 0.5D).add(Attributes.ATTACK_DAMAGE, 1.0D).build());
			GlobalEntityTypeAttributes.put(EntityInit.PSITTACOSAURUS_ENTITY.get(), MobEntity.createMobAttributes().add(Attributes.MAX_HEALTH, 10.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).build());
		});
	}
}

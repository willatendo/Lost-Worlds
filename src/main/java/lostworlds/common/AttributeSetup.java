...
public class AttributeSetup 
{
	...
	public static void registerEntityAttributes(FMLCommonSetupEvent event) 
	{
		event.enqueueWork(() -> 
		{
			...
			GlobalEntityTypeAttributes.put(EntityInit.LIAONINGOSAURUS_ENTITY.get(), MobEntity.createMobAttributes().add(Attributes.MAX_HEALTH, 10.0D).add(Attributes.MOVEMENT_SPEED, 0.3D).add(Attributes.ARMOR, 7.0D).add(Attributes.ARMOR_TOUGHNESS, 5.0D).build());

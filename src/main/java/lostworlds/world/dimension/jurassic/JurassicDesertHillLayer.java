...
public enum JurassicDesertHillLayer implements ICastleTransformer
{
	INSTANCE;
	
	private List<Integer> spawnBiome = ImmutableList.of(JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_DESERT_KEY));
	
	JurassicDesertHillLayer() { }
	...
	public int apply(INoiseRandom iNoiseRandom, int up, int left, int down, int right, int mid) 
	{
		...
		{
			return JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_DESERT_HILLS_KEY);
		}
		else
		{
			return -1;

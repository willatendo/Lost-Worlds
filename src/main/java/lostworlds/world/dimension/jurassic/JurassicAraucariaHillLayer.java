...
public enum JurassicAraucariaHillLayer implements ICastleTransformer
{
	INSTANCE;
	
	private List<Integer> spawnBiome = ImmutableList.of(JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_ARAUCARIA_FOREST_KEY));
	
	JurassicAraucariaHillLayer() { }
	...
	public int apply(INoiseRandom iNoiseRandom, int up, int left, int down, int right, int mid) 
	{
		...
		{
			return JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_ARAUCARIA_FOREST_HILLS_KEY);
		}
		else
		{
			return -1;

...
public enum JurassicConiferHillLayer implements ICastleTransformer
{
	INSTANCE;
	
	private List<Integer> spawnBiome = ImmutableList.of(JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_CONIFER_FOREST_KEY));
	
	JurassicConiferHillLayer() { }
	...
	public int apply(INoiseRandom iNoiseRandom, int up, int left, int down, int right, int mid) 
	{
		...
		{
			return JurassicLayerUtil.getBiomeId(BiomeInit.JURASSIC_CONIFER_FOREST_HILLS_KEY);
		}
		else
		{
			return -1;

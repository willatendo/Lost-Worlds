...
public enum PermianConiferHillLayer implements ICastleTransformer
{
	INSTANCE;
	
	private List<Integer> spawnBiome = ImmutableList.of(PermianLayerUtil.getBiomeId(BiomeInit.PERMIAN_CONIFER_FOREST_KEY));
	
	PermianConiferHillLayer() { }
	...
	public int apply(INoiseRandom iNoiseRandom, int up, int left, int down, int right, int mid) 
	{
		...
		{
			return PermianLayerUtil.getBiomeId(BiomeInit.PERMIAN_CONIFER_FOREST_HILLS_KEY);
		}
		else
		{
			return -1;

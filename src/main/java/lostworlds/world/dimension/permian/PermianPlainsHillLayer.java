...
public enum PermianPlainsHillLayer implements ICastleTransformer
{
	INSTANCE;
	
	private List<Integer> spawnBiome = ImmutableList.of(PermianLayerUtil.getBiomeId(BiomeInit.PERMIAN_PLAINS_KEY));
	
	PermianPlainsHillLayer() { }
	...
	public int apply(INoiseRandom iNoiseRandom, int up, int left, int down, int right, int mid) 
	{
		...
		{
			return PermianLayerUtil.getBiomeId(BiomeInit.PERMIAN_PLAINS_HILLS_KEY);
		}
		else
		{
			return -1;

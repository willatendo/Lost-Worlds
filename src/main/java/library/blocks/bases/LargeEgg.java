...
import library.util.base.ModBlockStateProperties;
...
public class LargeEgg extends Block
{
	private static final VoxelShape EGG_ONE = Block.box(9, 0, 7, 15, 10, 13);
	private static final VoxelShape EGG_TWO = Block.box(2, 0, 2, 15, 10, 13);
	...
	public LargeEgg(AbstractBlock.Properties properties, NonNullSupplier<? extends EntityType<? extends AbstractPrehistoricAnimalEntity>> entity) 
	{
		super(properties);
	...
	{
		return context.getItemInHand().getItem() == this.asItem() && state.getValue(EGGS) < 2 ? true : super.canBeReplaced(state, context);
	}
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) 
	{
		...
		return blockstate.is(this) ? blockstate.setValue(EGGS, Integer.valueOf(Math.min(2, blockstate.getValue(EGGS) + 1))) : super.getStateForPlacement(context);
	}

	@Override
	public VoxelShape getShape(BlockState state, IBlockReader reader, BlockPos pos, ISelectionContext context) 
	{
		return state.getValue(EGGS) == 2 ? EGG_TWO : EGG_ONE;
	}
	...
	{
		if(!(entity instanceof AbstractPrehistoricAnimalEntity) && !(entity instanceof BatEntity)) 
		{

...
public class SmallEggBlock extends Block 
{
	...
	public SmallEggBlock(AbstractBlock.Properties properties, NonNullSupplier<? extends EntityType<? extends AbstractPrehistoricAnimalEntity>> entity) 
	{
		super(properties);
	...
	{
		return state.getValue(EGGS) == 5 ? EGG_FIVE : state.getValue(EGGS) == 4 ? EGG_FOUR : state.getValue(EGGS) == 3 ? EGG_THREE : state.getValue(EGGS) == 2 ? EGG_TWO : EGG_ONE;
	}
